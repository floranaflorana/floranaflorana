...
; fiction story book
;(define zombie (string->url "http://textfiles.com/stories/zombies.txt"))

(define zombie (string->url "http://textfiles.com/stories/zombies.txt"))

;;; Open a connection port to the URL
(define in (get-pure-port zombie #:redirections 5))

;;; Next, we capture the text from our input port, removing capitalization, 
;;; punctuation, and then extra spaces
(define zombie-text (string-normalize-spaces
		   (remove-punctuation
...
(define words (document->tokens zombie-text #:sort? #t))
