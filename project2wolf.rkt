...
; fiction story book
;(define wolf (string->url "http://textfiles.com/stories/wolf7kid.txt"))

(define wolf (string->url "http://textfiles.com/stories/wolf7kid.txt"))

;;; Open a connection port to the URL
(define in (get-pure-port wolf #:redirections 5))

;;; Next, we capture the text from our input port, removing capitalization, 
;;; punctuation, and then extra spaces
(define wolf-text (string-normalize-spaces
		   (remove-punctuation
...
(define words (document->tokens wolf-text #:sort? #t))
