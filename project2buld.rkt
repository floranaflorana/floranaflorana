...
; fiction story book
;(define buld (string->url "http://textfiles.com/stories/buldream.txt"))

(define buld (string->url "http://textfiles.com/stories/buldream.txt"))

;;; Open a connection port to the URL
(define in (get-pure-port buld #:redirections 5))

;;; Next, we capture the text from our input port, removing capitalization, 
;;; punctuation, and then extra spaces
(define buld-text (string-normalize-spaces
		   (remove-punctuation
...
(define words (document->tokens buld-text #:sort? #t))
